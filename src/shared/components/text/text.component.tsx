...
export const Text = (props: RNTextProps & TextProps) => {
  const { underline = false, size = 14, color = Colors.BLACK, align, weight = 300, transform = 'none', style, onPress, children, maxLength, className } = props;
  ...
  return (
    ...
    >
      {displayedText}
      {maxLength && !isExpanded && (children as string).length > maxLength && <RNText style={styles.more}>...</RNText>}
    </RNText>
  );
};
