...
import { Input as GInput, InputField, InputIcon, InputSlot } from "../ui/input";
    ...
    >
      {leftIcon && (
        <InputSlot className="pl-3">
          {customLeftIcon || (icon && <InputIcon as={icon} />)}
        </InputSlot>
      )}
      ...
      {rightIcon && (
        <InputSlot
          style={styles.slot}
          onPress={() => secureProp && setSecureTextEntry(!secureTextEntry)}
        >
          <InputIcon
    ...
    >
      {label && (
        <FormControlLabel className="mb-4">
          <Text
            size={16}
            weight="400"
            color={error ? Colors.PRIMARY : Colors.TEXT}
          >
            {label}
          </Text>
        </FormControlLabel>
      ...
      {touched && error && (
        <View className="mt-2 items-start flex flex-row gap-2">
          {!isRequired && (
            <FormControlErrorIcon as={AlertCircleIcon} color={Colors.PRIMARY} />
          )}
          <Text
