...
import { useShift } from '@/src/shared/hooks/services';
  ...
  return (
    <>
      <View className="flex flex-1 w-full gap-2">
        <TurnHeader 
          zone_id={shift?.zone_id as string} 
        ...
        <View className="flex-1">
          ...
          <HStack className="w-full justify-around">
            <SummaryBlock 
              value={finalize_shift?.active_hours?.toString() || ''} 
              label="Horas activas" 
            />
            <SummaryBlock 
              value={`${finalize_shift?.total_distance || 0}km`} 
          ...
          <HStack className="w-full justify-around mt-8">
            <View className="flex-col gap-6">
              <InfoBlock 
                ...
                value={formatDate(finalize_shift?.start_time, finalize_shift?.start_date)} 
              />
              <InfoBlock 
                icon={Calendar} 
                label="Fecha de finalización" 
                value={formatDate(finalize_shift?.end_time, finalize_shift?.end_date)} 
              />
            </View>
            <View className="flex-col gap-6">
              <InfoBlock 
                ...
                value={formatTime(finalize_shift?.start_time || '', finalize_shift?.start_date)} 
              />
              <InfoBlock 
                icon={Clock} 
                label="Hora de finalización" 
                value={formatTime(finalize_shift?.end_time || '', finalize_shift?.end_date)} 
              />
            </View>
          </HStack>
        </View>
